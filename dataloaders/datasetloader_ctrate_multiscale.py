...
class CTMultiScaleDataset(Dataset):
    def __init__(self, config, data_dir, label_csv=None):
    ...
    def __getitem__(self, idx):
        ...
        if self.has_labels:
            # 1. CT-RATE format (e.g., valid_1_a_1.nii.gz)
...
def get_wds_dataset(config, tar_pattern=None):
...
def get_npy_validation_dataset(config, data_dir, label_csv, max_patients=200, seed=42):
    ...
    Returns:
        CTMultiScaleDataset filtered to the selected patients.
    """
    import re
    import random
    ...
    if not patient_to_unique_scans:
    ...
    # Select up to max_patients patients
    random.seed(seed)
    ...
    def patched_getitem(idx):
        ...
        if dataset.has_labels:
            # Extract patient ID and build the CSV lookup key
            match = patient_pattern.match(filename)
            if match:
                patient_id = match.group(1)
                csv_key = f"valid_{patient_id}.nii.gz"
            else:
